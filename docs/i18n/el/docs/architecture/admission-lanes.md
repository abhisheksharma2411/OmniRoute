...
## 1. Καθολικός ανά διεργασία έλεγχος εισδοχής σε επίπεδο byte (`chatBodyAdmission.ts`)
...
- **Πεδίο εφαρμογής:** η διαδρομή προσωρινά αποθηκευμένου σώματος/σωρού για `POST /v1/chat/completions`,
  ...
  από τη διόγκωση του σωρού λόγω μεγάλων σωμάτων από coding agents (#4380).
- **Ένας καθολικός ελεγκτής ανά διεργασία, όχι λωρίδες ανά κλειδί (#10110).** Κάθε κλειδί API
  (κατακερματισμένο) ή συνεδρία `anonymous` διεκδικεί εισδοχή από τον **ίδιο** κοινόχρηστο προϋπολογισμό —
  το κατακερματισμένο αναγνωριστικό συνεδρίας χρησιμοποιείται ΜΟΝΟ ως κλειδί δίκαιου προγραμματισμού (κυκλική
  κατανομή μεταξύ αιτημάτων σε αναμονή), ποτέ ως διαμέρισμα χωρητικότητας. Μια προηγούμενη έκδοση αυτού του
  εγγράφου περιέγραφε λωρίδες ανά κλειδί με ανεξάρτητη χωρητικότητα· αυτό το μοντέλο
  καταργήθηκε στο #10110, επειδή επέτρεπε σε πλαστά, μη αυθεντικοποιημένα διαπιστευτήρια να πολλαπλασιάζουν
  το καθολικό όριο της διεργασίας.
- **Πύλη (#503-fanout): ένας αυτόματα υπολογιζόμενος προϋπολογισμός εισαγωγής σε BYTE, όχι ένα σταθερό πλήθος
  αιτημάτων.** Το παλαιό ανώτατο όριο πλήθους αιτημάτων `CHAT_MAX_HEAVY_IN_FLIGHT` (προεπιλογή `1`
  πριν από αυτήν τη διόρθωση) περιόριζε το fan-out των coding agents (πολλαπλοί subagents/CLI,
  με σώματα συνήθως > 256 KB) σε πραγματική παραλληλία περίπου 1, προκαλώντας αποκρίσεις 503
  υπό απολύτως φυσιολογικό φόρτο. Πλέον εφαρμόζεται μόνο όταν ένας διαχειριστής ορίζει ρητά
  το `OMNIROUTE_CHAT_MAX_HEAVY_IN_FLIGHT`. Αν παραμείνει μη ορισμένο, η εισδοχή ελέγχεται αντ’ αυτού
  από το `OMNIROUTE_CHAT_MAX_INFLIGHT_BYTES` — έναν προϋπολογισμό που υπολογίζεται αυτόματα από το
  πραγματικό όριο μνήμης της διεργασίας (`src/shared/middleware/admissionBudget.ts`):
  ...
  2 GiB. Οι ρητές παρακάμψεις χρησιμοποιούν τα ίδια όρια. Αυτό προσαρμόζεται αυτόματα από ένα
  container 512 MB έως έναν επιτραπέζιο υπολογιστή 32 GB, χωρίς ρύθμιση μεταβλητών περιβάλλοντος. Ένα σώμα που δεν μπορεί
  να χωρέσει εντός του πραγματικού προϋπολογισμού απορρίπτεται αμέσως με `413 body_exceeds_budget`·
  μόνο ο ανταγωνισμός μεταξύ σωμάτων που μπορούν να εξυπηρετηθούν μεμονωμένα εισέρχεται στην οριοθετημένη
  ουρά δίκαιης εξυπηρέτησης. Ένας ενεργός ανιχνευτής πίεσης πόρων πολλαπλών σημάτων (αναλογία σωρού V8,
  cgroup, PSI, συμβάντα OOM — `open-sse/utils/resourcePressurePolicy.ts`) συντομεύει
  τον οριοθετημένο χρόνο αναμονής υπό `high` πίεση και απορρίπτει αμέσως με
  `503 resource_pressure` υπό `critical` πίεση, πριν ακόμη εισαχθεί οποιοδήποτε byte.
- **Ρύθμιση:**
  - `OMNIROUTE_CHAT_MAX_INFLIGHT_BYTES` — παράκαμψη του αυτόματα υπολογιζόμενου προϋπολογισμού byte
  ...
  - `OMNIROUTE_CHAT_ADMISSION_MAX_QUEUED_BYTES` — βαλβίδα σωρού για τα byte σε αναμονή (προεπιλογή 4 MB)
  - `OMNIROUTE_CHAT_VIRTUAL_TTL_MS` / `OMNIROUTE_CHAT_VIRTUAL_MAX_SESSIONS` — καταργημένες
    λειτουργίες χωρίς αποτέλεσμα από το #10110 και μετά (γίνονται δεκτές για συμβατότητα ρυθμίσεων, αλλά αγνοούνται)
- **Αναφορές:** `GET /api/monitoring/health` → `chatAdmission` (#11244) — συμπεριλαμβανομένων
  ...
  (false σε μια προεπιλεγμένη ανάπτυξη — επιβεβαιώνει ότι αυτό που εφαρμόζεται στην πράξη είναι ο προϋπολογισμός byte και όχι το παλαιό
  ανώτατο όριο πλήθους).
