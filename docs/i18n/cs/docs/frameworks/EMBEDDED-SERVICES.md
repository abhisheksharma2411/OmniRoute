...
> **Cílová skupina:** Vývojáři, kteří přidávají, udržují nebo ladí vestavěné služby (9Router, CLIProxyAPI, Mux, Bifrost).

Vestavěné služby jsou lokálně nainstalované doprovodné procesní nástroje, které OmniRoute instaluje, spravuje a
zpřístupňuje jako plnohodnotné cíle směrování. Na rozdíl od externích poskytovatelů (ke kterým se přistupuje přes internet
pomocí klíčů API) běží vestavěné služby na stejném počítači jako OmniRoute a komunikují přes rozhraní loopback.
...
### Proč vestavěné služby?

Vestavěno je pět služeb:

| Služba          | Balíček npm                                     | Výchozí port | Účel                                                                                                                                                                                                |
| --------------- | ----------------------------------------------- | :----------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **9Router**     | `9router`                                       |    20130     | Směrovač AI, který může OmniRoute používat jako dílčího poskytovatele. Modely jsou zpřístupněny jako `9router/{sub}/{model}`                                                                        |
| **CLIProxyAPI** | Binární soubor z vydání na GitHubu (`cliproxy`) |     8317     | Lokální proxy adaptér pro autentizační toky CLI Anthropic. Poskytuje záložní směrování po vypršení platnosti tokenů OAuth                                                                           |
| **Mux**         | `mux` (bezobslužný `mux server`)                |     8322     | Lokální démon pro orchestraci agentů (coder/mux). Pouze správa životního cyklu — nejde o cíl směrování (bez proxy LLM).                                                                             |
| **Bifrost**     | `@maximhq/bifrost`                              |     8080     | Backend přenosové brány AI v jazyce Go. Pokud je spuštěný, je automaticky vybrán trasou přenosu (`/v1/relay/`)                                                                                      |
| **Dario**       | `@askalf/dario`                                 |     3456     | Proxy předplatného Claude — alternativa/záloha k CLIProxyAPI pro provoz ve formátu Claude Code; vložený klíč se stane `DARIO_ADMIN_TOKEN`, který řídí přístup k jeho řídicí vrstvě OAuth `/admin/*` |

Všech pět používá stejný model dohledu:

- OmniRoute je instaluje do `DATA_DIR/services/{name}/` (odděleně od vlastního souboru `package.json` služby OmniRoute)
- OmniRoute je spouští a monitoruje jako podřízené procesy
- OmniRoute vkládá do prostředí podřízeného procesu dočasný klíč API a obměňuje jej bez odstávky (kde je to možné)
- Všechny řídicí trasy (`/api/services/*`) jsou **LOCAL_ONLY** — přístupné pouze z rozhraní loopback (pevné pravidlo č. 17)

### Klíčová rozhodnutí (z návrhového plánu)

| Rozhodnutí                                                         | Hodnota                                                                                                |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| Přístup řídicího panelu k nativnímu uživatelskému rozhraní 9Router | Reverzní proxy na `/dashboard/providers/services/9router/embed/*`                                      |
| Mechanismus instalace                                              | `npm install {package}` prostřednictvím `execFile` (bez interpolace shellu)                            |
| Režim využití                                                      | Poskytovatel zaregistrovaný ve směrovacím modulu jako `9router/{sub}/{model}`                          |
| Správa klíčů API                                                   | OmniRoute je generuje, šifruje při uložení (AES-256-GCM) a vkládá prostřednictvím proměnných prostředí |
| Umístění v řídicím panelu                                          | `/dashboard/providers/services` (tři karty)                                                            |
...
│  Živé logy (SSE), Spustit/Zastavit/Restartovat/Aktualizovat, Nastavení, Instalace│
│                                                                    │
│  src/app/(dashboard)/dashboard/providers/services/                 │
│    ├── page.tsx               Obálka + směrování karet pomocí ?tab=│
...
│    ├── logs:       kruhový buffer stdio o velikosti 5 MB → události SSE│
│    ├── health:     HTTP GET /health každé 2–5 s, líné zotavení     │
...
│    Před předáním požadavku odstraní z ID modelu předponu "9router/".│
...
│    Modely jsou uloženy jako "9router/{sub}/{model}" (s předponou). │
│    Synchronizace probíhá každých 5 min pomocí modelSync.ts.        │
│                                                                    │
│  Mux má spravovaný POUZE životní cyklus (vrstvy 1–3) — jde o démona│
│  pro orchestraci agentů, nikoli proxy LLM, takže nemá exekutor ani │
...
| `src/lib/services/registry.ts`              | Singletonová mapa `nástroj → správce`                        |
| `src/lib/services/apiKey.ts`                | Generování klíčů, šifrování AES-256-GCM pro uložená data     |
| `src/lib/services/modelSync.ts`             | Pravidelná synchronizace modelů (5 min) + na vyžádání        |
| `src/lib/services/ringBuffer.ts`            | Kruhový buffer protokolů o velikosti 5 MB s odběrem přes SSE |
| `src/lib/services/healthCheck.ts`           | HTTP kontrola stavu (nastavitelný interval)                  |
| `src/lib/services/installers/ninerouter.ts` | Instalace/aktualizace/odinstalace 9Router přes npm           |
| `src/lib/services/installers/cliproxy.ts`   | Instalace/aktualizace/odinstalace CLIProxyAPI přes npm       |
| `src/lib/services/installers/mux.ts`        | Instalace/aktualizace/odinstalace Mux přes npm               |
...
Požadavky mimo loopback obdrží `403 LOCAL_ONLY` bez ohledu na ověřovací token.
...
`package.json` a `node_modules/`. Nedochází ke konfliktu s vlastními závislostmi OmniRoute.
...
| `500` | Instalace npm selhala — uživatelsky přívětivou chybu najdete v `message` |

**Poznámky:** Používá `execFile('npm', [...])` — bez shellu a bez interpolace (pevné pravidlo č. 13).
Chyby EACCES se zobrazují jako uživatelsky přívětivé zprávy.

---

#### `POST /api/services/9router/start`

Spustí 9Router. Pokud supervisor ještě není zaregistrován, zaregistruje jej a poté
zavolá `supervisor.start()`. Pokud již služba běží, je operace idempotentní.
...
Řádně zastaví 9Router. Odešle SIGTERM, počká 15 s a poté odešle SIGKILL, pokud proces stále běží.
...
Ekvivalent volání `stop()` následovaného `start()` pod zámkem operací.
...
poté se spustí instalace npm (novější verze se nainstaluje na stejné místo) a následně
se služba restartuje.
...
Vygeneruje nový API klíč pro 9Router, zašifruje jej v úložišti a restartuje službu
(pokud je spuštěna), aby načetla nový klíč ze svého prostředí. Platnost starého klíče
je okamžitě zrušena.
...
| `500` | Rotace se nezdařila                        |

**Zabezpečení:** Nový klíč není nikdy vrácen v odpovědi (nedojde k úniku přihlašovacích údajů).
...
Vrací kombinovaný aktuální stav a stav z DB, včetně metadat verze a náhledu API klíče.
...
| ----- | -------------------------- |
| `200` | Viz schéma níže            |
| `500` | Načtení stavu se nezdařilo |
...
Přepne příznak automatického spuštění. Když je `enabled: true`, služba se automaticky
spustí při příštím spuštění OmniRoute (pokud je služba nainstalována).
...
SSE stream aktuálních protokolů z kruhové vyrovnávací paměti stdout/stderr služby 9Router.
...
| `filter` | `string`  | žádný           | Filtr podřetězce bez rozlišení velikosti písmen (bez regexu — bezpečné proti ReDoS) |

**Události SSE:**

| Událost     | Data        | Popis                               |
| ----------- | ----------- | ----------------------------------- |
| `snapshot`  | `LogLine[]` | Počáteční konec historického výpisu |
| `log`       | `LogLine`   | Aktuální řádek protokolu            |
| `heartbeat` | `{}`        | Udržovací signál každých 15 s       |
...
CLIProxyAPI má stejnou strukturu koncových bodů jako 9Router, ale bez `rotate-key`; navíc má
`accounts`, `provider-expose` a `auto-restart-adopted`. Nyní dostává
vyhrazený API klíč datové vrstvy vložený při spuštění (`needsApiKey: true` v
`bootstrap.ts`, používá se k synchronizaci modelů); `status` obsahuje méně polí.

| Metoda | Cesta                               | Popis                                          |
| ------ | ----------------------------------- | ---------------------------------------------- |
| `POST` | `/api/services/cliproxy/install`    | Nainstaluje CLIProxyAPI z npm                  |
| `POST` | `/api/services/cliproxy/start`      | Spustí CLIProxyAPI                             |
| `POST` | `/api/services/cliproxy/stop`       | Zastaví CLIProxyAPI                            |
| `POST` | `/api/services/cliproxy/restart`    | Restartuje CLIProxyAPI                         |
| `POST` | `/api/services/cliproxy/update`     | Aktualizuje na novější verzi                   |
| `GET`  | `/api/services/cliproxy/status`     | Aktuální stav + stav z DB (bez `apiKeyMasked`) |
| `POST` | `/api/services/cliproxy/auto-start` | Přepne automatické spuštění                    |

Sdílený koncový bod `GET /api/services/{name}/logs` (viz §4.1) funguje pro všechny
čtyři služby a používá dynamický segment `[name]`.

---

### 4.3 Koncové body Mux (8 tras)

Mux má stejnou strukturu koncových bodů jako CLIProxyAPI — v rozhraní API nemá trasu
`rotate-key` (nosný token se generuje stejným způsobem jako u 9Router prostřednictvím
`getOrCreateApiKey("mux")` a vkládá se pomocí proměnné prostředí `MUX_SERVER_AUTH_TOKEN`, ale
zatím pro něj neexistuje vyhrazený koncový bod pro rotaci). U Mux se spravuje pouze životní cyklus:
na rozdíl od 9Router nemá executor vrstvy 4 a nikdy není registrován jako poskytovatel směrování.

| Metoda | Cesta                          | Popis                               |
| ------ | ------------------------------ | ----------------------------------- |
| `POST` | `/api/services/mux/install`    | Nainstaluje Mux z npm (`npm i mux`) |
| `POST` | `/api/services/mux/start`      | Spustí Mux (`mux server`)           |
| `POST` | `/api/services/mux/stop`       | Zastaví Mux                         |
| `POST` | `/api/services/mux/restart`    | Restartuje Mux                      |
| `POST` | `/api/services/mux/update`     | Aktualizuje na novější verzi npm    |
| `GET`  | `/api/services/mux/status`     | Aktuální stav + stav z DB           |
| `POST` | `/api/services/mux/auto-start` | Přepne automatické spuštění         |

---

### 4.4 Koncové body Bifrost (8 tras)

Bifrost je backend přenosové brány AI napsaný v Go (`@maximhq/bifrost`). Používá stejnou
strukturu koncových bodů jako CLIProxyAPI (bez `rotate-key` — Bifrost spravuje vlastní klíče
poskytovatelů v souboru `config.json` ve svém adresáři `-app-dir`).

| Metoda | Cesta                              | Popis                                                                            |
| ------ | ---------------------------------- | -------------------------------------------------------------------------------- |
| `POST` | `/api/services/bifrost/install`    | Instalace Bifrostu z npm (`@maximhq/bifrost`)                                    |
| `POST` | `/api/services/bifrost/start`      | Spuštění Bifrostu na portu 8080 (výchozí)                                        |
| `POST` | `/api/services/bifrost/stop`       | Zastavení Bifrostu                                                               |
| `POST` | `/api/services/bifrost/restart`    | Restartování Bifrostu                                                            |
| `POST` | `/api/services/bifrost/update`     | Aktualizace na novější verzi                                                     |
| `GET`  | `/api/services/bifrost/status`     | Aktuální stav + stav v DB                                                        |
| `POST` | `/api/services/bifrost/auto-start` | Přepnutí automatického spouštění                                                 |
| `GET`  | `/api/services/bifrost/logs`       | Průběžný výpis protokolu přes SSE (pomocí sdílené dynamické trasy `[name]/logs`) |

**Nastavení směrování:** Pokud proměnná `BIFROST_BASE_URL` není nastavena a spravovaná
instance Bifrostu je spuštěná, funkce `getBifrostRoutingConfig()` (v souboru `routingBackend.ts`) automaticky
použije `http://127.0.0.1:{port}` jako základní URL přenosového serveru. Explicitně nastavená proměnná prostředí
`BIFROST_BASE_URL` má vždy přednost.

---

### 4.5 Koncové body Dario (12 tras)

Stejná struktura životního cyklu jako u ostatních služeb (`install`, `start`, `stop`, `restart`,
`update`, `status`, `auto-start`, `auto-restart-adopted`) a navíc řídicí vrstva OAuth
chráněná tokenem pod `admin/`: `admin/accounts`, `admin/import-from-omniroute`,
`admin/login-start`, `admin/login-complete` (všechny chráněné pomocí `DARIO_ADMIN_TOKEN`).

### 4.6 Reverzní proxy (vložený ovládací panel 9Router)

Ovládací panel vkládá webové uživatelské rozhraní 9Router do prvku iframe prostřednictvím interní
reverzní proxy na adrese:
...
- Odstraňuje příchozí hlavičky `cookie` a `authorization` (zabrání úniku relace OmniRoute)
- Vkládá hlavičku `Authorization: Bearer {apiKey}` pro ověření 9Router
- Z odpovědi odstraňuje `set-cookie`, `content-security-policy`, `x-frame-options` a `cross-origin-*`
- Přepisuje odpovědi HTML tak, aby vložila `<base href>` a normalizovala absolutní cesty (`/foo` → `/dashboard/.../embed/foo`)

Přechody na protokol WebSocket pro vložený ovládací panel zpracovává doprovodný server na
vyhrazeném portu (viz `src/lib/services/embedWsProxy.ts`).

**Zabezpečení:** Trasy vložené proxy jsou zařazeny pod `LOCAL_ONLY_API_PREFIXES`
a jsou přístupné pouze z místní smyčky. Útočník, který získá JWT prostřednictvím
tunelu Cloudflare/Ngrok, nemůže proxy využít pro přístup k vloženým službám.
