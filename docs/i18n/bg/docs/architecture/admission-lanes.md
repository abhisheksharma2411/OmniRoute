...
## 1. Допускане на ниво байтове за целия процес (`chatBodyAdmission.ts`)

- **Обхват:** пътят на буферираното тяло/heap за `POST /v1/chat/completions`,
  `/v1/messages`, `/v1/responses` и останалите маршрути, структурирани като чат. Предпазва
  от увеличаване на използването на heap вследствие на големи тела от агенти за програмиране (#4380).
- **Един глобален контролер за процеса, а не отделни канали за всеки ключ (#10110).** Всеки API ключ
  ...
  разпределяне между изчакващите), никога като сегмент на капацитета. Предишна версия на този
  документ описваше отделни канали за всеки ключ с независим капацитет; този модел беше
  премахнат в #10110, защото позволяваше чрез фалшиви неудостоверени идентификационни данни да се
  умножи ограничението за целия процес.
- **Ограничител (#503-fanout): автоматично изведен БАЙТОВ бюджет за приемане, а не фиксиран брой
  заявки.** Остарялото ограничение по брой заявки `CHAT_MAX_HEAVY_IN_FLIGHT` (по подразбиране `1`
  преди тази корекция) свеждаше паралелното разпределение на агентите за програмиране (множество подагенти/CLI,
  тела обичайно > 256 KB) до ефективна конкурентност от ~1, което водеше до отговори 503
  ...
  25% от по-строгия лимит измежду лимита на V8 heap и всеки лимит на cgroup/контейнера,
  разделени на коефициент 8x за временно увеличаване, ограничени между 8 MiB и
  2 GiB. Изрично зададените стойности използват същите граници. Това се мащабира автоматично от
  контейнер с 512 MB до настолен компютър с 32 GB без настройване чрез променливи на средата. Тяло, което не може
  да се побере в ефективния бюджет, се отхвърля незабавно с `413 body_exceeds_budget`;
  само конкуренцията между тела, които поотделно могат да бъдат обслужени, влиза в ограничената
  справедлива опашка. Механизъм за проследяване в реално време на натиска върху ресурсите чрез множество сигнали (съотношение на V8 heap,
  cgroup, PSI, OOM събития — `open-sse/utils/resourcePressurePolicy.ts`) съкращава
  ограниченото изчакване при `high` натиск и незабавно отхвърля натоварването с
  `503 resource_pressure` при `critical` натиск, още преди да бъдат приети каквито и да е
  байтове.
- **Настройване:**
  - `OMNIROUTE_CHAT_MAX_INFLIGHT_BYTES` — заместваща стойност за автоматично изведения байтов бюджет
  - `OMNIROUTE_CHAT_MAX_HEAVY_IN_FLIGHT` — остаряло ограничение по брой заявки, само при изрично включване
  - `OMNIROUTE_CHAT_ADMISSION_QUEUE_MS` — изчакване в опашката преди 503 (по подразбиране 2000)
  - `OMNIROUTE_CHAT_ADMISSION_MAX_QUEUED_BYTES` — предпазен клапан за heap според байтовете в опашката (по подразбиране 4 MB)
  - `OMNIROUTE_CHAT_VIRTUAL_TTL_MS` / `OMNIROUTE_CHAT_VIRTUAL_MAX_SESSIONS` — остарели
    операции без действие след #10110 (приемат се за съвместимост на конфигурацията, но се игнорират)
- **Отчети:** `GET /api/monitoring/health` → `chatAdmission` (#11244) — включително
  ...
  (false при внедряване по подразбиране — потвърждава, че реално ограничава байтовият бюджет, а не остарялото
  ограничение по брой).
