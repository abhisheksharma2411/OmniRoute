...
## 1. پذیرش سراسری پردازه در سطح بایت (`chatBodyAdmission.ts`)

- **دامنه:** مسیر بدنهٔ بافرشده/حافظهٔ heap برای `POST /v1/chat/completions`،
  `/v1/messages`، `/v1/responses` و سایر مسیرهای مشابه چت. از تقویت مصرف
  حافظهٔ heap ناشی از بدنههای بزرگ عاملهای کدنویسی جلوگیری میکند (#4380).
- **یک کنترلگر سراسری برای کل پردازه، نه مسیرهای مجزا برای هر کلید (#10110).** هر کلید API
  (بهصورت هششده) یا نشست `anonymous` برای پذیرش از **همان** بودجهٔ مشترک استفاده
  میکند — شناسهٔ هششدهٔ نشست فقط بهعنوان کلید زمانبندی منصفانه (توزیع
  round-robin میان درخواستهای منتظر) استفاده میشود و هرگز نقش یک بخش مستقل
  از ظرفیت را ندارد. نسخهٔ قبلی این سند، مسیرهای جداگانه برای هر کلید با ظرفیت
  مستقل را توصیف میکرد؛ آن مدل در #10110 حذف شد، زیرا به اعتبارنامههای جعلی
  و احرازهویتنشده اجازه میداد حد سراسری پردازه را چندبرابر کنند.
- **دروازه (#503-fanout): بودجهٔ BYTE برای ورود داده که بهطور خودکار محاسبه میشود، نه تعداد
  ثابت درخواستها.** سقف قدیمی تعداد درخواست `CHAT_MAX_HEAVY_IN_FLIGHT` (با مقدار
  پیشفرض `1` پیش از این اصلاح) fan-out عاملهای کدنویسی (چندین زیرعامل/CLI،
  با بدنههایی که معمولاً بزرگتر از 256 KB هستند) را به همزمانی مؤثر حدود ۱
  کاهش میداد و در بار کاملاً عادی به خطای 503 منجر میشد. اکنون این سقف فقط
  زمانی اعمال میشود که اپراتور صراحتاً `OMNIROUTE_CHAT_MAX_HEAVY_IN_FLIGHT` را
  تنظیم کند. اگر تنظیم نشده باشد، پذیرش در عوض توسط
  `OMNIROUTE_CHAT_MAX_INFLIGHT_BYTES` کنترل میشود — بودجهای که بهطور خودکار
  از سقف واقعی حافظهٔ پردازه محاسبه میشود
  (`src/shared/middleware/admissionBudget.ts`): ۲۵٪ از مقدار محدودتر میان حد
  heap در V8 و هرگونه محدودیت cgroup/container، تقسیم بر ضریب 8x برای افزایش
  موقت مصرف حافظه و محدودشده به بازهٔ 8 MiB تا 2 GiB. مقادیر override صریح
  نیز از همین حدود استفاده میکنند. این بودجه بدون نیاز به تنظیم متغیرهای
  محیطی، خود را از یک container با حافظهٔ 512 MB تا یک رایانهٔ رومیزی با حافظهٔ
  32 GB مقیاس میدهد. بدنهای که در بودجهٔ مؤثر جای نگیرد، بلافاصله با
  `413 body_exceeds_budget` رد میشود؛ فقط رقابت میان بدنههایی که بهصورت
  مستقل قابل سرویسدهی هستند وارد صف محدود و منصفانه میشود. یک ردیاب زندهٔ
  فشار منابع با چند سیگنال (نسبت heap در V8،‏ cgroup،‏ PSI، رویدادهای OOM —
  `open-sse/utils/resourcePressurePolicy.ts`) در فشار `high` زمان انتظار
  محدود را کوتاه میکند و در فشار `critical`، حتی پیش از ورود هرگونه بایت،
  درخواست را فوراً با `503 resource_pressure` حذف میکند.
- **تنظیمات:**
  - `OMNIROUTE_CHAT_MAX_INFLIGHT_BYTES` — override برای بودجهٔ بایتی محاسبهشده بهصورت خودکار
  - `OMNIROUTE_CHAT_MAX_HEAVY_IN_FLIGHT` — سقف قدیمی تعداد درخواستها که فقط با opt-in فعال میشود
  - `OMNIROUTE_CHAT_ADMISSION_QUEUE_MS` — مدت انتظار در صف پیش از 503 (پیشفرض 2000)
  - `OMNIROUTE_CHAT_ADMISSION_MAX_QUEUED_BYTES` — شیر کنترل heap برای بایتهای صفشده (پیشفرض 4 MB)
  - `OMNIROUTE_CHAT_VIRTUAL_TTL_MS` / `OMNIROUTE_CHAT_VIRTUAL_MAX_SESSIONS` — از
    #10110 منسوخ شده و بدون اثر هستند (برای سازگاری پیکربندی پذیرفته میشوند، اما نادیده گرفته میشوند)
- **گزارشها:** `GET /api/monitoring/health` → `chatAdmission` (#11244) — شامل
  ...
  تعداد درخواستها، محدودیت واقعی اعمالشده است).
