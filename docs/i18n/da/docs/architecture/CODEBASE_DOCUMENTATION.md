...
## 4. `open-sse/` — arbejdsområde til streamingmotoren

Separat npm-arbejdsområde udgivet som `@omniroute/open-sse`. Håndterer forespørgselsbehandling, eksekutorer, oversættere, tjenester, transformer og MCP-serveren.
...
├── handlers/               Forespørgselshåndterere (chat, embeddings, lyd, billeder, …)
...
| Håndterer               | Formål                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------- |
| `chatCore.ts`           | Primær chatpipeline (cache, hastighedsbegrænsning, kombinationsrouting, afsendelse til eksekutor) |
...
| `usageExtractor.ts`     | Udtrækker antal tokens fra opstrøms-streams                                                       |
| `responseSanitizer.ts`  | Fjerner udbyderspecifik støj                                                                      |
| `responseTranslator.ts` | Forbindelsesled mellem udbydersvar og oversætterlaget                                             |
...
> OpenAI-kompatible eksekutor. Det komplette udbyderkatalog (355 udbydere) findes i
...
- **9 forespørgselsoversættere** (`translator/request/`):
...
- Øverste niveau: `bootstrap.ts`, `formats.ts`, `registry.ts`, `index.ts`.
...
| Robusthed          | `accountFallback.ts` (nedkølingsperiode + spærring), `errorClassifier.ts`, `requestRejectedStreak.ts`, `emergencyFallback.ts`, `rateLimitManager.ts`, `rateLimitSemaphore.ts`, `accountSemaphore.ts`, `accountSelector.ts`                        |
...
- **33 scopes** håndhæves under kørsel — grundlisten findes i `src/shared/constants/mcpScopes.ts`, og det fulde sæt er unionen af de scopes, som hvert værktøjsmodul deklarerer.
...
`providerHeaderProfiles.ts`), modelregistre pr. format (`audioRegistry.ts`,
