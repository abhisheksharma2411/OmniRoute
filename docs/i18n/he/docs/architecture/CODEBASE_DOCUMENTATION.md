...
בקשות, מריצים, מתרגמים, שירותים, ממיר ושרת MCP.
...
├── services/               יותר מ-80 מודולי שירות (שילובים, חלופה, מכסות, זהות, …)
...
| מטפל                    | מטרה                                                                  |
| ----------------------- | --------------------------------------------------------------------- |
| `chatCore.ts`           | צינור עיבוד הצ'אט הראשי (מטמון, הגבלת קצב, ניתוב שילובים, שיגור מריץ) |
...
108 מריצי ספקים, שכל אחד מהם מרחיב את `BaseExecutor` (`base.ts`):
...
- `responsesTransformer.ts` — ממיר מבוסס-`TransformStream` בין Responses API ל-Chat
  Completions (משמש את נתיב ברירת המחדל `responses/`).
...
| מודיעין ניתוב   | `intentClassifier.ts`, `taskAwareRouter.ts`, `backgroundTaskDetector.ts`, `volumeDetector.ts`, `wildcardRouter.ts`, `workflowFSM.ts`, `specificityDetector.ts`, `specificityRules.ts`, `specificityTypes.ts`                                      |
...
- **33 היקפי הרשאה** נאכפים בזמן ריצה — רשימת הבסיס נמצאת ב-`src/shared/constants/mcpScopes.ts`, והקבוצה המלאה היא איחוד היקפי ההרשאה המוצהרים בכל מודול כלים.
