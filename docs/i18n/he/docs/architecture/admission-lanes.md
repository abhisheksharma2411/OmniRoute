...
## 1. בקרת כניסה כלל־תהליכית ברמת הבתים (`chatBodyAdmission.ts`)

- **תחום תחולה:** נתיב גוף־הבקשה המאוחסן במטמון/ערימה עבור `POST /v1/chat/completions`,
  `/v1/messages`, `/v1/responses` ושאר הנתיבים בעלי מבנה צ'אט. מגן
  מפני הגברת השימוש בערימה עקב גופי בקשה גדולים של סוכני קידוד (#4380).
- **בקר גלובלי יחיד לכל תהליך, ולא מסלולים נפרדים לכל מפתח (#10110).** כל מפתח API
  (מגובב) או הפעלה מסוג `anonymous` מתקבלים כנגד **אותו** תקציב משותף —
  מזהה ההפעלה המגובב משמש **אך ורק** כמפתח לתזמון הוגן (שיגור בשיטת סבב
  בין הממתינים), ולעולם לא כפלח קיבולת. גרסה קודמת של מסמך זה
  תיארה מסלולים נפרדים לכל מפתח עם קיבולת עצמאית; מודל זה
  הוסר ב-#10110 משום שהוא אפשר לפרטי הזדהות מזויפים ולא מאומתים
  להכפיל את המגבלה הכלל־תהליכית.
- **שער (#503-fanout): תקציב בתים לקליטה הנגזר אוטומטית, ולא מספר קבוע של
  בקשות.** מגבלת מספר הבקשות הישנה `CHAT_MAX_HEAVY_IN_FLIGHT` (ברירת המחדל הייתה `1`
  לפני תיקון זה) צמצמה פיצול עומס של סוכני קידוד (מספר סוכני משנה/ממשקי שורת פקודה,
  עם גופים הגדולים באופן שגרתי מ-256 KB) למקביליות אפקטיבית של כ-1, דבר שגרם
  לתגובות 503 בעומס רגיל לחלוטין. כעת היא מחייבת רק כאשר מפעיל מגדיר במפורש
  את `OMNIROUTE_CHAT_MAX_HEAVY_IN_FLIGHT`. כאשר היא אינה מוגדרת, בקרת הכניסה
  נשלטת במקום זאת באמצעות `OMNIROUTE_CHAT_MAX_INFLIGHT_BYTES` — תקציב הנגזר אוטומטית
  מתקרת הזיכרון האמיתית של התהליך (`src/shared/middleware/admissionBudget.ts`):
  25% מהערך המחמיר יותר מבין מגבלת הערימה של V8 לבין כל מגבלת cgroup/קונטיינר,
  מחולקים במקדם הגברה זמנית של 8x, ומוגבלים לטווח שבין 8 MiB לבין
  2 GiB. דריסות מפורשות משתמשות באותן מגבלות. התקציב מתאים את עצמו
  מקונטיינר של 512 MB ועד למחשב שולחני עם 32 GB, ללא כוונון של משתני סביבה. גוף בקשה שאינו
  יכול להיכלל בתקציב האפקטיבי נכשל מיד עם `413 body_exceeds_budget`;
  רק תחרות בין גופי בקשה שכל אחד מהם ניתן לטיפול בנפרד נכנסת לתור ההוגנות
  המוגבל. עוקב חי אחר לחץ משאבים המבוסס על אותות מרובים (יחס ערימת V8,
...
- **כוונון:**
  ...
  - `OMNIROUTE_CHAT_MAX_HEAVY_IN_FLIGHT` — מגבלת מספר בקשות ישנה, בהצטרפות יזומה בלבד
...
- **דיווח:** `GET /api/monitoring/health` → `chatAdmission` (#11244) — כולל
  ...
  (`false` בפריסה המוגדרת כברירת מחדל — מאשר שתקציב הבתים, ולא מגבלת
