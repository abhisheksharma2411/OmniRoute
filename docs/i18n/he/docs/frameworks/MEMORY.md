...
| `vectorStore`            | `"sqlite-vec" \| "qdrant" \| "auto"`               | `"auto"`   | באיזה מנגנון וקטורי להשתמש                              |
...
מבלי לשנות את מרשם ההטמעות הגלובלי. נקודת הקצה מנורמלת לפני השימוש ונבדקת
בהתאם למדיניות כתובות ה-URL היוצאות של הספק: נדרש HTTP(S), פרטי זיהוי מוטמעים
ומחרוזות שאילתה נדחים, וכתובות מטא-נתונים של שירותי ענן נותרות חסומות. ערכים
ריקים משמרים את ספק המרשם שנבחר. שגיאות המוחזרות ללוח הבקרה עוברות טיהור,
ופרטי הזיהוי של נקודת הקצה לעולם אינם נרשמים ביומן.

> **TODO (D20):** הטווח `global` (שיתוף זיכרונות בין כל מפתחות ה-API) אינו
