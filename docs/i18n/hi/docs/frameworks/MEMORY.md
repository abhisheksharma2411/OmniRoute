...
`src/shared/schemas/memory.ts` में परिभाषित हैं और `src/lib/db/settings.ts` के माध्यम से स्थायी रूप से सहेजे जाते हैं:

| फ़ील्ड                   | प्रकार                                             | डिफ़ॉल्ट | विवरण                                                         |
| ------------------------ | -------------------------------------------------- | -------- | ------------------------------------------------------------- |
| `embeddingSource`        | `"remote" \| "static" \| "transformers" \| "auto"` | `"auto"` | उपयोग किया जाने वाला एम्बेडिंग स्रोत                          |
| `embeddingProviderModel` | `string \| null`                                   | `null`   | `provider/model` प्रारूप में प्रोवाइडर/मॉडल                   |
| `customBaseUrl`          | `string \| null`                                   | `null`   | केवल मेमोरी के लिए OpenAI-संगत एंडपॉइंट बेस URL               |
| `customModelId`          | `string \| null`                                   | `null`   | कस्टम एंडपॉइंट को भेजी जाने वाली मॉडल ID                      |
| `transformersEnabled`    | `boolean`                                          | `false`  | Transformers.js (MiniLM, ~400MB) के लिए ऑप्ट-इन               |
| `staticEnabled`          | `boolean`                                          | `false`  | स्थिर potion-base-8M स्थानीय मॉडल के लिए ऑप्ट-इन              |
| `rerankEnabled`          | `boolean`                                          | `false`  | री-रैंकिंग चरण सक्षम करें (प्रति अनुरोध +200-500ms जोड़ता है) |
| `rerankProviderModel`    | `string \| null`                                   | `null`   | `provider/model` प्रारूप में री-रैंक प्रोवाइडर/मॉडल           |
...
`customModelId` सेटिंग्स भी स्वीकार करता है। ये दोनों मिलकर वैश्विक एम्बेडिंग रजिस्ट्री को बदले बिना OpenAI-संगत `/embeddings`
एंडपॉइंट और मॉडल का चयन करते हैं। उपयोग से पहले एंडपॉइंट को
सामान्यीकृत किया जाता है और प्रोवाइडर की आउटबाउंड URL नीति द्वारा जाँचा जाता है: HTTP(S)
आवश्यक है, एम्बेड किए गए क्रेडेंशियल और क्वेरी स्ट्रिंग अस्वीकार किए जाते हैं, और क्लाउड-मेटाडेटा
पते अवरुद्ध रहते हैं। रिक्त मान चयनित रजिस्ट्री प्रोवाइडर को बनाए रखते हैं। डैशबोर्ड को
लौटाई गई त्रुटियों को सैनिटाइज़ किया जाता है और एंडपॉइंट क्रेडेंशियल कभी लॉग नहीं किए जाते।

> **TODO (D20):** `global` स्कोप (सभी API कुंजियों में मेमोरी साझा करना) इस रिलीज़ में
> लागू नहीं किया गया है। इसके लिए स्कीमा परिवर्तन और एक वैश्विक पुनर्प्राप्ति
