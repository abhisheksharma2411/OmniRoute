...
├── handlers/               Rukovatelji zahtjevima (razgovor, ugrađivanja, zvuk, slika, …)
...
├── services/               Više od 80 modula usluga (kombinacije, pričuvni mehanizmi, kvote, identitet, …)
├── utils/                  Pomoćni alati za strujanje, TLS klijent, AWS SigV4, dohvaćanje putem proxyja, …
...
| Rukovatelj              | Svrha                                                                                                       |
| ----------------------- | ----------------------------------------------------------------------------------------------------------- |
| `chatCore.ts`           | Glavni cjevovod razgovora (predmemorija, ograničenje brzine, usmjeravanje kombinacija, otprema izvršitelju) |
...
| `sseParser.ts`          | Raščlanjivač SSE događaja                                                                                   |
...
108 izvršitelja pružatelja, od kojih svaki proširuje `BaseExecutor` (`base.ts`):

`antigravity`, `azure-openai`, `blackbox-web`, `cliproxyapi`,
`chatgpt-web-codex`, `cloudflare-ai`, `codex`, `commandCode`, `cursor`, `default`, `devin-cli`,
`muse-spark-web`, `nlpcloud`, `opencode`, `perplexity-web`, `petals`,
`pollinations`, `qoder`, `vertex`, `devin-desktop`, kao i `claudeIdentity.ts`
(dijeljeni pomoćni alat za identitet) i `index.ts` (registar).

> Napomena: pružatelje koji ovdje nisu navedeni poslužuje `default.ts` pomoću generičkog
> izvršitelja kompatibilnog s OpenAI-jem. Potpuni katalog pružatelja (355 pružatelja) nalazi se u
> `src/shared/constants/providers.ts`.

### 4.3 `open-sse/translator/`

Prevođenje prema modelu čvorišta i krakova (OpenAI je čvorište).
...
- **9 prevoditelja odgovora** (`translator/response/`):
  ...
  `openai-to-claude`.
- **9 pomoćnih alata** (`translator/helpers/`):
  `claudeHelper`, `geminiHelper`, `geminiToolsSanitizer`, `maxTokensHelper`,
  `openaiHelper`, `responsesApiHelper`, `schemaCoercion`, `toolCallHelper`, kao i
  testovi pomoćnih alata.
- **Pomoćni alati za slike** (`translator/image/sizeMapper.ts`).
- Najviša razina: `bootstrap.ts`, `formats.ts`, `registry.ts`, `index.ts`.

### 4.4 `open-sse/transformer/`

- `responsesTransformer.ts` — pretvarač Responses API ↔ Chat Completions temeljen
  na `TransformStream` (upotrebljava ga sveobuhvatna ruta `responses/`).

### 4.5 `open-sse/services/`

Najvažnije komponente (potpuni popis nalazi se u `open-sse/services/`):
...
| Tokeni i sesije           | `tokenRefresh.ts`, `sessionManager.ts`, `apiKeyRotator.ts`, `contextManager.ts`, `contextHandoff.ts`, `systemPrompt.ts`, `roleNormalizer.ts`, `responsesInputSanitizer.ts`, `toolSchemaSanitizer.ts`, `toolLimitDetector.ts`, `thinkingBudget.ts` |
...
| Skupna obrada             | `batchProcessor.ts`                                                                                                                                                                                                                               |
| Upotreba                  | `usage.ts`                                                                                                                                                                                                                                        |

### 4.6 `open-sse/mcp-server/`

- **110 jedinstvenih alata** povezano je u `server.ts` (45 kanonskih u `schemas/tools.ts` +
  moduli za memoriju, vještine, GitHub vještine, skupove, gamifikaciju, dodatke, Notion, Obsidian,
  lokalni korpus i kompresiju — unija izbrojena funkcijom `countUniqueMcpTools`).
- **3 transporta**: stdio, HTTP Streamable, SSE.
- **33 opsega** provode se tijekom izvođenja — osnovni popis nalazi se u `src/shared/constants/mcpScopes.ts`, a potpuni skup predstavlja uniju opsega koje deklarira svaki modul alata.
...
- Cjelovit katalog alata potražite u dokumentu [MCP-SERVER.md](../frameworks/MCP-SERVER.md).
...
Primitivi za streaming i pomoćne funkcije pružatelja usluga: `stream.ts`, `streamHandler.ts`,
